...
1. ES6编码风格 ([阮一峰es6](http://es6.ruanyifeng.com/))

2. 统一使用`4`个空格缩进、`UTF-8` 编码

3. 文件末尾添加新行

4. 注释使用 `//` 作为注释符，可以使用 `/* */` 作为多行注释符。
> 注释符号与注释内容之间留空，注释的位置尽量放在代码之上

5. 变量定义不用下划线`_` , 常量定义要大写

6. 比较用`===` /`!==`

7. 大括号不另起一行 `if(xxx){ `

8. 函数定义使用表达式 不使用声明式 避免变量提升
...
([eslint规则官网](http://eslint.cn/docs/rules/))

> 第一部分是规则名 ,第二部分是表示级别：0-不验证；1-警告；2-错误 具体看如下
